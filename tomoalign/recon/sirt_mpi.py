...
class SIRT(object):
    ...
    def _initialize(self):
        ...
        self.my_W = sparse.csr_matrix.dot(
            self.proj_mat, np.ones((self.geometry.n_vox,), dtype=self.precision)
        )
        self.my_V = sparse.csc_matrix.dot(
            sparse.csr_matrix.transpose(self.proj_mat),
        ...
        self.V[self.V < 1.0e-8] = np.inf
        self.my_W[self.my_W < 1.0e-8] = np.inf
        self.V = 1.0 / self.V
        self.my_W = 1.0 / self.my_W

    def run_main_iteration(
        self, niter=100, positivity=False, make_plot=False, debug=False
    ):
        ...
        while k < niter and not stop:
            ...
            my_back_proj = sparse.csc_matrix.dot(
                sparse.csr_matrix.transpose(self.proj_mat), self.my_W * res.ravel()
            )
            ...
            if self.ground_truth is None:
                ...
                self.rms_error[k] = (
                    np.linalg.norm(self.ground_truth - self.rec) / norm_factor
                )
            ...
            if debug:
                print(
                    "in rank %d: time taken for one SIRT iteration for %d projections = %4.5f"
                    % (self.my_rank, self.my_n_proj, time.time() - t_start)
                )

        return self.rec.reshape(self.geometry.vox_shape), self.rms_error[:k]

    def run_regularized_gradient_descent(
        self, niter=100, reg_param=1.0, positivity=False, make_plot=False, debug=False
    ):
