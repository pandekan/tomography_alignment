...
from tomoalign.utils import projection_operators, tv_denoise
...
class RegularizedRecon(object):
    ...
    def run_tikhonov_gd(
        ...
        while k < niter and not stop:
            ...
            res = self.projections - sparse.csr_matrix.dot(
            ...
            cost = 0.5 * (
                np.linalg.norm(res) ** 2 + reg_param * np.linalg.norm(self.rec) ** 2
            )
            alpha, _, _ = line_search_armijo(
                my_tikh_f,
                self.rec,
                -grad,
                grad,
            ...
            if self.ground_truth is None:
                ...
                rms_error[k] = (
                    np.linalg.norm(self.ground_truth - self.rec) / self.norm_factor
                )
    ...
    def run_lasso_ista(
        ...
        while k < niter and not stop:
            ...
            _, alpha, success = self._backtrack_lasso(
                alpha0, beta, res, grad, reg_param
            )
            step_size[k] = alpha
            if not success:
    ...
    def _backtrack_lasso(self, t, beta, g0, dg0, _lambda):
        ...
        while t > 1.0e-16:
            ...
            g = (
                sparse.csr_matrix.dot(self.proj_mat, xp).reshape(self.n_proj, -1)
                - self.projections
            )
...
def my_tikh_fp(x, proj_mat, b, _lambda):
    res = sparse.csr_matrix.dot(proj_mat, x) - b
    grad = (
        sparse.csc_matrix.dot(sparse.csr_matrix.transpose(proj_mat), res) + _lambda * x
    )
