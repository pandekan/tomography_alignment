...
def forward_sparse(geometry, alpha, beta, phi, xyz_shift):
    ...
    rot_centers = rigid_transformation(geometry.vox_centers, alpha, beta, phi, xyz_shift)
    ...
    dat_inds, det_inds, wts, n_inds = vox_wt_grad.bilinear_sparse(geometry.n_vox,
...
def forward_proj_grad(geometry, alpha, beta, phi, xyz_shift, rec):
    ...
    det_img, gradient = vox_wt_grad.bilinear_vox_interp(geometry.n_vox, np.asfortranarray(floor_x),
                                                        np.asfortranarray(floor_z),
                                                        np.asfortranarray(alpha_x),
                                                        np.asfortranarray(alpha_z),
