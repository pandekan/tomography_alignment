...
from tomoalign.utils.ray_voxel_utilities import (
    forward_proj_grad as ray_forward_proj_grad,
)
from tomoalign.utils.ray_voxel_utilities import forward_sparse as ray_forward_sparse
from tomoalign.utils.voxel_utilities import forward_proj_grad as vox_forward_proj_grad
from tomoalign.utils.voxel_utilities import forward_sparse as vox_forward_sparse
...
class ProjectionMatrix(object):
    ...
    def projection_matrix(
        self, alpha=None, beta=None, phi=None, xyz_shift=None, voxel_mask=None
    ):
        if phi is None:
            self.n_proj = self.geometry.n_proj
            phi = np.linspace(0.0, np.pi, self.n_proj)
        else:
    ...
    def _forward_voxel(self):
        ...
        for iproj in range(self.n_proj):
            ...
            dat_inds, inds, wts = vox_forward_sparse(
                this_geo, alpha, beta, phi, xyz_shift
            )
    ...
    def _forward_ray(self):
        ...
        for iproj in range(self.n_proj):
            ...
            dat_inds, det_inds, wts = ray_forward_sparse(
                this_geo, alpha, beta, phi, xyz_shift
            )
    ...
    def projection_gradient(self, rec, alpha, beta, phi, xyz_shift, cor_shift):
        ...
        proj_img, gradient = ray_forward_proj_grad(
            this_geo, alpha, beta, phi, xyz_shift, rec
        )
