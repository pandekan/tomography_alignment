...
from tomoalign.recon import regularized_mpi
from tomoalign.utils import generate_phantom, geometry, projection_operators
...
pmat = proj_obj.projection_matrix(
    alpha=alpha[my_proj_index],
    beta=beta[my_proj_index],
    phi=phi[my_proj_index],
    xyz_shift=xyz[my_proj_index],
)
my_proj[my_proj_index] = sparse.csr_matrix.dot(pmat, ground_truth.ravel()).reshape(
    my_n_proj, -1
)
...
rec_obj = regularized_mpi.RegularizedRecon(
    comm,
    geom,
...
if penalty == "Tikh":
    # Tikhonov penalized
    reg_tv = 0.1
    rec, err = rec_obj.run_tikhonov_gd(
        niter=niter_rec, reg_param=reg_tv, positivity=True, make_plot=True
    )
elif penalty == "Lasso":
    # |x|_1 penalised with iterative soft-thresholding
    reg_ista = 1.0
    beta_ista_ls = 0.8
    rec, err = rec_obj.run_lasso_accelerated(
        niter=niter_rec, reg_param=reg_ista, beta=beta_ista_ls, make_plot=True
    )
elif penalty == "TV":
    # TV penalised with FISTA
    hyper = 1.0e4
    beta_tv = 0.1
    rec, err = rec_obj.run_fista(
        niter=niter_rec, hyper=hyper, beta_tv=beta_tv, make_plot=True
    )
else:
    if rank == 0:
