...
class SIRT(object):
    ...
    def run_main_iteration(
        ...
        while k < niter and not stop:
            ...
            back_proj = sparse.csc_matrix.dot(
                sparse.csr_matrix.transpose(self.proj_mat), self.W * res.ravel()
            )
            ...
            if k > 0 and k % 20 == 0 and debug:
                print(
                    "time taken for 20 SIRT iterations = %4.5f"
                    % (time.time() - t_start)
                )
                t_start = time.time()
    ...
    def run_regularized_gradient_descent(
        self, niter=100, reg_param=1.0, positivity=True, make_plot=False, debug=False
    ):
