...
def cross_correlation_skimage(projections, sinogram_order="True"):
    ...
    for i in range(1, n_proj):
        shifts, error, phase_diff = pcc(
            aligned_proj[i - 1], aligned_proj[i], upsample_factor=100
        )
        offsets[i, :] = shifts
...
def cross_correlation_numpy(projections):
    ...
    for i in range(1, n_proj):
        offsets[i, :], aligned_proj[i] = crossCorrelationAlign(
            aligned_proj[i], aligned_proj[i - 1], filter_r, filter_k
        )
