...
import copy
import sys
from utilities import geometry, projection_operators, alignment_functions
from recon import sirt
from utilities.alignment_functions import cost_xzab, gradient_xzab
...
for it in range(1, niter_align):
    sirt_obj = sirt.SIRT(geom, proj.reshape(n_proj, -1), np.array([phi, alpha_rec[it-1], beta_rec[it-1]]).T,
            xyz_rec[it-1], options={'ground_truth':ground_truth, 'rec':rec.ravel()})
    rec, err = sirt_obj.run_main_iteration(niter=niter_rec, positivity=True, make_plot=True)
    for i in range(n_proj):
        ...
        params = np.array([xyz_rec[it,i,0], xyz_rec[it,i,2], alpha_rec[it,i], beta_rec[it,i]])
        res = optimize.minimize(cost_xzab, params, method='L-BFGS-B',
                jac=gradient_xzab, args=(align_obj, rec, np.array([phi[i], 0.0, 0.0]), np.zeros(3,)),
                bounds=((-3., 3.), (-3.0, 3.0), (-0.02, 0.02), (-0.02, 0.02)),
                options={'disp':False})
        ...
        print('%4.5f, %4.5f, %4.5f, %4.5f, %4.5f'
                %(xyz_rec[it-1,i,0], xyz_rec[it-1,i,2], np.rad2deg(alpha_rec[it-1,i]), 
                    np.rad2deg(beta_rec[it-1,i]), residual[it-1,i]))
        print('%4.5f, %4.5f, %4.5f, %4.5f, %4.5f'
