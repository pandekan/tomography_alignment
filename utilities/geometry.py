...
class Geometry(object):
    ...
    def _geo_parameters(self, angles=None, shifts=None):
        ...
        if angles is None:
            ...
            self.angles[0] = np.linspace(0., np.pi, self.n_proj)
    ...
    def _voxel_detector_grid(self):
        ...
        self.source_centers = np.array([xd.ravel(), y_source*np.ones((self.n_det, )), zd.ravel()])
        self.det_centers = np.array([xd.ravel(), y_det*np.ones((self.n_det, )), zd.ravel()])
        
        # this information is needed for voxel based method
        self.det_orig = np.array([x.min(), y.min(), z.min()])
        sx, sz = float(self.vox_shape[0]/self.det_shape[0]), float(self.vox_shape[2]/self.det_shape[1])
        self.factor = np.array([sx, 1., sz])
