subroutine forward_project(alpha, beta, phi, xyz, cor_shift, source_points, detector_points, origin, &
        step_size, recon, n_proj, n_rays, nx, ny, nz, ax)
    
    implicit none
    
    integer(kind=4),                         intent(in)  :: n_proj, n_rays, nx, ny, nz
    ...
    do np = 1, n_proj
        ...
        s = source_points
        d = detector_points
        s(1, :) = s(1, :) + cor_shift(1, np)
        d(1, :) = d(1, :) + cor_shift(1, np)
        call rigid_transformation(s, alpha(np), beta(np), phi(np), xyz(:, np), n_rays, p0)
        call rigid_transformation(d, alpha(np), beta(np), phi(np), xyz(:, np), n_rays, p1)
